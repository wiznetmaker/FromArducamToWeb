...
static void http_process_handler(uint8_t s, st_http_request * p_http_request)
{
	...
	switch (p_http_request->METHOD)
	{
		...
		case METHOD_GET :
			...
			uri_name = uri_buf;
			// Check for the /camera-frame endpoint
			...
			if (!strcmp((char *)uri_name, "capture")) strcpy((char *)uri_name, "capture.html");
		...
		case METHOD_POST :
			...
			if(p_http_request->TYPE == PTYPE_CGI)	// HTTP POST Method; CGI Process
			{
...
#endif
				if(!camera_flag && content_found && (file_len <= (DATA_BUF_SIZE-(strlen(RES_CGIHEAD_OK)+8))))
				{
				...
				else if(content_found && camera_flag)
				{
					send_http_response_camcgi(s, pHTTP_TX, http_response, file_len);
					camera_flag = 0;
				}
				else
				{
